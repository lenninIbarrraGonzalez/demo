...
import type { PlantillaFormulario, CampoFormulario } from '@/types';
...
export default function EditarFormularioPage() {
  ...
  const [formulario, setFormulario] = useState<PlantillaFormulario | null>(null);
  ...
  useEffect(() => {
    ...
    const formularioData = storage.getPlantillaFormularioById(formularioId);
    if (formularioData) {
      setFormulario(formularioData);
      setNombre(formularioData.nombre);
      setDescripcion(formularioData.descripcion);
      setVersion(formularioData.version);
      setCampos(formularioData.campos);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (campos.length === 0) {
        toast.error('Debes tener al menos un campo');
        setIsSubmitting(false);
        return;
      }

      const updated = storage.updatePlantillaFormulario(formularioId, {
        nombre: nombre.trim(),
  ...
  const handleDelete = async () => {
    try {
      const deleted = storage.deletePlantillaFormulario(formularioId);
      if (deleted) {
  ...
  return (
    <DashboardLayout sections={SUPER_ADMIN_NAV} title="Editar Formulario">
